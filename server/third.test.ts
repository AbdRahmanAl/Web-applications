...
test("After creating a book, the new book should be in the list of books", async () => {
  const book: Book = {category: "Arabic", pages: []};
...
test("If we try to add a page to a book that does not exist, the server sends a message that says the book does not exist", async () => {
  expect(await bookService.addPage(username, "Pizza", [""], "Western")).toBe(
    "Book does not exist"
  );
});
